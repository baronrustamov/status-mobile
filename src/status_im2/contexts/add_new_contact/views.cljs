  ...
  (let [clipboard     (reagent/atom nil)
    ...
    (fn []
      (let [{:keys [input
            ...
            _ (clipboard/get-string #(reset! clipboard %))
            show-paste-button? (and @clipboard
                ...
                (merge style/button-paste
                       {:on-press
                        (fn []
                          ...
                          (rf/dispatch [:contacts/set-new-identity @clipboard]))})
                (i18n/label :t/paste)])]
            [quo/button
